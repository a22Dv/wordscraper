...
WSR_EXCEPTMSG(cPosErrMsg) = "Could not retrieve current cursor position. Using last-known coordinate...";
...
INPUT Input::createMInput_(int x, int y, int flags) {
  WSR_ASSERT(x >= 0 && y >= 0);
...
Input::Input() {
  POINT cPos = {};
  BOOL gcRt = GetCursorPos(&cPos);
  utils::windowsRequire(gcRt, WSR_EXCEPTION(cPosErrMsg));
  cx_ = cPos.x;
  cy_ = cPos.y;
...
void Input::moveMouseTo(int x, int y, std::chrono::milliseconds duration) {
  ...
  POINT start = {};
  BOOL rt = GetCursorPos(&start);
  if (!rt) [[unlikely]] {
    utils::logMessage(utils::LogSeverity::LOG_ERROR, WSR_SIGNATURE(cPosErrMsg));
    start.x = cx_;
    start.y = cy_;
  }
  ...
  for (int i = 1; i <= segments; ++i) {
    ...
    INPUT moveInput = createMInput_(int(start.x + i * dXPerSegment), int(start.y + i * dYPerSegment), MOUSEEVENTF_MOVE);
    UINT sRt = SendInput(1, &moveInput, sizeof(INPUT));
    if (!sRt) [[unlikely]] {
      utils::logMessage(utils::LogSeverity::LOG_ERROR, WSR_SIGNATURE(sInErrMsg));
    }
